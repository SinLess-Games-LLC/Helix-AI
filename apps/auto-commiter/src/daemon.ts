...
export class Daemon {
  ...
  constructor() {
    this.logger = new Logger({ serviceName: 'AutoCommiter-Daemon', fluentd: {enabled: false} });
  ...
  private async runLoop() {
    ...
    watcher.on('all', async (event, path) => {
      ...
      this.debouncedHandleFileChange = setTimeout(() => this.handleFileChange(), 1000); // Debounce for 1 second
    });
  ...
  private async handleFileChange() {
    try {
      ...
      if (this.lastCommit && now.getTime() - this.lastCommit.getTime() < 300000) {
        this.logger.info(`Less than 5 minutes since last commit. Skipping commit...`);
        return;
      }
      ...
      while (attempts < maxAttempts && !commitValid) {
        this.logger.info(`Attempt ${attempts + 1} to generate commit message...`);
      ...
      if (!commitValid) {
        this.logger.error('All attempts to generate a valid commit message failed. Waiting 5 minutes...');
    ...
    try {
      ...
      const payload = {
        ...
        messages: [
          ...
          { role: 'system', content: "Commitlint types: ['feat', 'fix', 'docs', 'style', 'refactor', 'test', 'chore', 'revert']" },
  ...
  private async validateCommitMessage(message: string): Promise<boolean> {
    try {
      const { stderr } = await execAsync(`echo "${message}" | npx commitlint --verbose`);
      if (stderr) {
