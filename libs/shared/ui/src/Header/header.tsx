...
export const Header: React.FC<HeaderProps> = ({ logo, title, version, pages, style, menuOpen, toggleMenu }) => {
  ...
  useEffect(() => {
    ...
    const checkDisplayPages = () => {
      ...
      const totalPagesWidth = pages.reduce((total, page) => total + page.name.length * 8, 0); // Approx width per character
  ...
  return (
    <header className="header" style={style}>
      ...
      <nav className="header__nav">
        <ul className={`header__nav-list ${menuOpen ? 'header__nav-list--open' : ''} ${canDisplay ? '' : 'hidden'}`}>
          {/* Display page links if they fit within 1/3rd of the header */}
          {canDisplay && pages.map((page: Page, index) => (
            <li key={index}>
